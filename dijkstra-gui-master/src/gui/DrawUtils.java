...
public class DrawUtils {
    ...
    public static void drawSourceNode(Node node){
        g.setColor(parseColor("#00BCD4"));
        g.fillOval(node.getX() - radius, node.getY() - radius, 2 * radius, 2 * radius);
    ...
    //    System.out.println("drawSou");
    }

    public static void drawCurrentNode(Node node){
        g.setColor(parseColor("#4caf50"));
        g.fillOval(node.getX() - radius, node.getY() - radius, 2 * radius, 2 * radius);

        radius-=5;
        g.setColor(parseColor("#c8e6c9"));
        g.fillOval(node.getX() - radius, node.getY() - radius, 2 * radius, 2 * radius);
    ...
    public static void drawDestinationNode(Node node){
    ...
    public static void drawCentreText(String text, int x, int y) {
        FontMetrics fm = g.getFontMetrics();
        double t_width = fm.getStringBounds(text, g).getWidth();
        g.drawString(text, (int) (x - t_width / 2), (y + fm.getMaxAscent() / 2));
    }
